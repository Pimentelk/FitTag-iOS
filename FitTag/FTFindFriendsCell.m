...
@implementation FTFindFriendsCell
@synthesize delegate;
@synthesize user;
...
#pragma mark - FTFindFriendsCell

- (void)setUser:(PFUser *)aUser {
    user = aUser;
    
    // Configure the cell
    [avatarImageView setFile:[self.user objectForKey:kFTUserProfilePicSmallKey]];
    
    // Set name
    NSString *nameString = [self.user objectForKey:kFTUserDisplayNameKey];
    CGSize nameSize = [nameString boundingRectWithSize:CGSizeMake(144.0f, CGFLOAT_MAX)
                                            ...
                                            attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:16.0f]}
                                               context:nil].size;
    [nameButton setTitle:[self.user objectForKey:kFTUserDisplayNameKey] forState:UIControlStateNormal];
    [nameButton setTitle:[self.user objectForKey:kFTUserDisplayNameKey] forState:UIControlStateHighlighted];
